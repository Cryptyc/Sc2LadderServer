...
ExitCase GameUpdate(sc2::Connection *client, sc2::Server *server, const std::string *botName, uint32_t MaxGameTime,
                    uint32_t MaxRealGameTime, float_t *AvgFrame, int32_t *GameLoop) {
    ...
    try {
            ...
            if (server->HasRequest()) {
                ...
                if (request.second) {
                    if (request.second->has_quit()) //Really paranoid here...
                    {
                        ...
                        PrintThread{} << *botName
                                      << " IS USING DEBUG INTERFACE.  POSSIBLE CHEAT Please tell them not to"
                                      << std::endl;
                        AlreadyWarned = true;
        ...
        *AvgFrame = AvgStepTime;
        PrintThread{} << *botName << " Exiting with " << GetExitCaseString(CurrentExitCase) << " Average step time "
                      << AvgStepTime << std::endl;
...
bool LadderManager::ProcessObservationResponse(SC2APIProtocol::ResponseObservation Response,
    ...
    if (Response.player_result_size()) {
        ...
        for (const auto &player_result : Response.player_result()) {
            PlayerResults->push_back(sc2::PlayerResult(player_result.player_id(),
                                                       sc2::ConvertGameResultFromProto(player_result.result())));
        }
        return true;
    }
    return false;
...
std::string LadderManager::GetBotCommandLine(const BotConfig &AgentConfig, int GamePort, int StartPort,
                                             const std::string &OpponentId, bool CompOpp, sc2::Race CompRace,
                                             sc2::Difficulty CompDifficulty) {
    ...
    switch (AgentConfig.Type) {
        ...
        case DefaultBot: {
        } // BlizzardAI - doesn't need any command line arguments
    }

    // Add universal arguments
    OutCmdLine += " --GamePort " + std::to_string(GamePort) + " --StartPort " + std::to_string(StartPort) +
                  " --LadderServer 127.0.0.1 --OpponentId " + OpponentId;

    if (CompOpp) {
        OutCmdLine += " --ComputerOpponent 1 --ComputerRace " + GetRaceString(CompRace) + " --ComputerDifficulty " +
                      GetDifficultyString(CompDifficulty);
    }
    if (AgentConfig.Args != "") {
...
sc2::GameRequestPtr CreateStartGameRequest(const std::string &MapName, std::vector <sc2::PlayerSetup> players,
                                           sc2::ProcessSettings process_settings) {
    sc2::ProtoInterface proto;
    sc2::GameRequestPtr request = proto.MakeRequest();
...
GameResult
LadderManager::StartGameVsDefault(const BotConfig &Agent1, sc2::Race CompRace, sc2::Difficulty CompDifficulty,
    ...
    int32_t GameLoop;
    auto bot1UpdateThread = std::async(GameUpdate, &client, &server, &Agent1.BotName, MaxGameTime, MaxRealGameTime,
                                       &AvgFrameTime, &GameLoop);
    sc2::SleepFor(1000);
    ...
    std::string ReplayFile =
        ReplayDir + Agent1.BotName + "v" + GetDifficultyString(CompDifficulty) + "-" + RemoveMapExtension(Map) +
        ".Sc2Replay";
...
GameResult LadderManager::StartGame(const BotConfig &Agent1, const BotConfig &Agent2, const std::string &Map) {
    ...
    int32_t GameLoop;
    auto bot1UpdateThread = std::async(&GameUpdate, &client, &server, &Agent1.BotName, MaxGameTime, MaxRealGameTime,
                                       &Bot1AvgFrame, &GameLoop);
    auto bot2UpdateThread = std::async(&GameUpdate, &client2, &server2, &Agent2.BotName, MaxGameTime, MaxRealGameTime,
                                       &Bot2AvgFrame, nullptr);
    sc2::SleepFor(1000);
    ...
    std::string ReplayFile =
        ReplayDir + Agent1.BotName + "v" + Agent2.BotName + "-" + RemoveMapExtension(Map) + ".SC2Replay";
    ReplayFile.erase(remove_if(ReplayFile.begin(), ReplayFile.end(), isspace), ReplayFile.end());
    ...
    if (CurrentResult == Player1Crash || CurrentResult == Player2Crash) {
        ...
        try {
            ...
            PrintThread{} << e.what() << std::endl << " Unable to detect end of update thread.  Continuing"
                          << std::endl;
        }
...
LadderManager::LadderManager(int InCoordinatorArgc, char **inCoordinatorArgv)
    ...
    : coordinator(nullptr), CoordinatorArgc(InCoordinatorArgc), CoordinatorArgv(inCoordinatorArgv), MaxGameTime(0),
      MaxRealGameTime(0), MaxEloDiff(0), ConfigFile("LadderManager.json"), EnableReplayUploads(false),
      EnableServerLogin(false), EnablePlayerIds(false), Sc2Launched(false), Config(nullptr), PlayerIds(nullptr) {
}
...
LadderManager::LadderManager(int InCoordinatorArgc, char **inCoordinatorArgv, const char *InConfigFile)
    ...
    : coordinator(nullptr), CoordinatorArgc(InCoordinatorArgc), CoordinatorArgv(inCoordinatorArgv), MaxGameTime(0),
      MaxRealGameTime(0), MaxEloDiff(0), ConfigFile(InConfigFile), EnableReplayUploads(false),
      EnableServerLogin(false), EnablePlayerIds(false), Sc2Launched(false), Config(nullptr), PlayerIds(nullptr) {
}
...
void
LadderManager::SaveJsonResult(const BotConfig &Bot1, const BotConfig &Bot2, const std::string &Map, GameResult Result) {
...
void
LadderManager::ChangeBotNames(const std::string ReplayFile, const std::string &Bot1Name, const std::string Bot2Name) {
    std::string CmdLine = Config->GetValue("ReplayBotRenameProgram");
    if (CmdLine.size() > 0) {
        CmdLine =
            CmdLine + " " + ReplayFile + " " + FIRST_PLAYER_NAME + " " + Bot1Name + " " + SECOND_PLAYER_NAME + " " +
            Bot2Name;
...
bool LadderManager::UploadCmdLine(GameResult result, const Matchup &ThisMatch) {
    ...
    if (ThisMatch.Agent2.Type == BotType::DefaultBot) {
        ReplayFile =
            ThisMatch.Agent1.BotName + "v" + GetDifficultyString(ThisMatch.Agent2.Difficulty) + "-" + RawMapName +
            ".Sc2Replay";
    } else {
        ...
        for (const auto &val : Units.GetArray()) {
            if (val.HasMember("name") && val["name"].IsString()) {
                ...
                if (ThisBot != BotConfigs.end()) {
                    if (val.HasMember("deactivated") && val.HasMember("deleted") && val["deactivated"].IsBool() &&
                        val["deleted"].IsBool()) {
                        ...
                        } else if (val["deactivated"].GetBool() == false && val["deleted"].GetBool() == false &&
                                   ThisBot->second.Enabled == false) {
                            // reenable a bot
...
bool LadderManager::IsInsideEloRange(std::string Bot1Name, std::string Bot2Name) {
    ...
    if (Bot1 != BotConfigs.end() && Bot2 != BotConfigs.end()) {
        ...
        PrintThread{} << Bot1Name << " ELO: " << Bot1->second.ELO << " | " << Bot2Name << " ELO: " << Bot2->second.ELO
                      << " | Diff: " << EloDiff << std::endl;
    ...
    try {
        ...
        while (Matchups->GetNextMatchup(NextMatch)) {
            ...
            if (IsBotEnabled(NextMatch.Agent1.BotName) && IsBotEnabled(NextMatch.Agent2.BotName) &&
                IsInsideEloRange(NextMatch.Agent1.BotName, NextMatch.Agent2.BotName)) {
                GameResult result;
                PrintThread{} << std::endl << "Starting " << NextMatch.Agent1.BotName << " vs "
                              << NextMatch.Agent2.BotName << " on " << NextMatch.Map << std::endl;
                if (NextMatch.Agent1.Type == DefaultBot || NextMatch.Agent2.Type == DefaultBot) {
                    ...
                    result = StartGameVsDefault(NextMatch.Agent1, NextMatch.Agent2.Race, NextMatch.Agent2.Difficulty,
                                                NextMatch.Map);
                } else {
                    result = StartGame(NextMatch.Agent1, NextMatch.Agent2, NextMatch.Map);
                }
                PrintThread{} << std::endl << "Game finished with result: " << GetResultType(result.Result)
                              << std::endl;
