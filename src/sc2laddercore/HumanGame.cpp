...
HumanGame::HumanGame(std::string ReplayFile)
    : ReplayLocation(ReplayFile)
{
}

GameResult HumanGame::StartGame(const BotConfig& Agent1, const BotConfig& Agent2, const std::string& Map)
{
    // Proxy init
    ...
    {
        proxyBot1.startSC2InstanceHuman(process_settings, portServerBot1, portClientBot1);
    }
    else
    {
    ...
    if (ReplayLocation.back() != '/')
    {
        ReplayLocation += "/";
    }
    std::string replayFile = ReplayLocation + Agent1.BotName + "v" + Agent2.BotName + "-" + RemoveMapExtension(Map) + ".SC2Replay";
