...
struct GameState {
    ...
    GameState()
        : IsInGame(true), GameLoop(-1), Score(-1) {}
...
struct GameResult {
    ...
    GameResult()
        : Result(ResultType::InitializationError), Bot1AvgFrame(0), Bot2AvgFrame(0), GameLoop(0) {}
...
struct BotConfig {
    ...
    BotConfig()
        : Type(BotType::BinaryCpp), Race(sc2::Race::Random), Difficulty(sc2::Difficulty::Easy), Debug(false),
          Enabled(true), ELO(0) {}

    BotConfig(BotType InType, const std::string &InBotName, sc2::Race InBotRace, const std::string &InBotPath,
              const std::string &InFileName, sc2::Difficulty InDifficulty = sc2::Difficulty::Easy,
              const std::string &InArgs = "")
        : Type(InType), Race(InBotRace), BotName(InBotName), RootPath(InBotPath), FileName(InFileName),
          Difficulty(InDifficulty), Args(InArgs), Debug(false) {}
